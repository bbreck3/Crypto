...
void KeyExpansionCore(unsigned char *in4,unsigned int i){
	//Rotate Left
...
std::string  AES_Encrypt(std::string plaintext, unsigned char *expandedKey, unsigned int Nr){
...
std::string  AES_Decrypt(std::string ciphertext, unsigned char* expandedKey, unsigned short Nr)
{
...
std::vector<std::string> break_up(std::string plaintext){
    int counter=0;
    int total_counter=0;
    std::vector<std::string> plaintext_list;
    std::string temp_plaintext="";
    std::cout << "plaintext length: " << plaintext.length() << endl;
    for (int i = 0; i <plaintext.length(); ++i)
    {  
        //std::cout << "DEBUG\n" <<std::endl;
        if(counter==15){
            plaintext_list.push_back(temp_plaintext);
            counter=0;
            temp_plaintext.clear();
        }
        if((plaintext.length() - i)<16){
            std::string final_part = plaintext.substr(i+1,plaintext.length());
            int originalLen =  final_part.length();//strlen((const char*)message);
            int lenOfPaddedMessage = originalLen;
            if(lenOfPaddedMessage % 16 !=0){
               lenOfPaddedMessage = (lenOfPaddedMessage / 16 +1)*16;
            }       
            std::string paddedMessage(lenOfPaddedMessage,' '); //new unsigned char[lenOfPaddedMessage];
            for (int i = 0; i < lenOfPaddedMessage; ++i)
            {
                if (i>=originalLen)
                {
                    ...
                    paddedMessage[i]=final_part[i];
                }
            }
            plaintext_list.push_back(paddedMessage);
            //plaintext_list.push_back(final_part);
            return plaintext_list;
        }
        std::string temp;
        temp+=plaintext[i];
        temp_plaintext.append(temp); 
        counter++;
        total_counter++;
    }
    return plaintext_list;
...
int main(){
    ...
    ****************************************************/
    std::string plaintext = "This is a realy long message that will be encrypted with AES";
    cout << "plaintext = " << plaintext << endl;

    std::vector<std::string> plaintext_list = break_up(plaintext);
    for (auto ptx: plaintext_list)
    {
        cout << ptx << endl;
    }
    ...
    for(auto ptx: plaintext_list){
        ...
        ciphertext = AES_Encrypt(ptx, expandedKey, Nr);
        //std::string temp = "                     ";
        cout << "ciphertext = "<<ciphertext << endl;
        
        //cout << "ciphertext = " << ciphertext <<endl;
        ciphertext_list.push_back(ciphertext);
    }

    // decrypt
    //unsigned char decrypted_plaintext[stt_lng];
    for(auto ctx: ciphertext_list){
        //std::cout << "DEBUG" << std::endl;
        std::string decrypted_plaintext = ctx;
        decrypted_plaintext = AES_Decrypt(ctx, expandedKey, Nr);
